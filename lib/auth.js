  ...
  if (req.session && req.session.userid) {
    ...
    cas.validate(req.query.ticket, function (err, casresponse, result) {
      ...
      if (result.validated) {
        ...
        debug(req.session.userid + ' and ticket ' + req.query.ticket + ' is validated');
        ...
        ldapClient.search(ad.searchBase, opts, false, function (err, result) {
          ...
          debug(req.session.userid + ' found in AD');
          ...
          User.findOne({
            ...
            if (user) {
              debug(req.session.userid + ' found in DB');
...
function verifyRole(role) {
  return function (req, res, next) {
    if (req.session.roles) {
      ...
      log.log('Cannot find the user\'s role.');
