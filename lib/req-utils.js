 ...
 * @param  {String} type          the type of request, params, body, query
 *                                default: params
 * @param  {String} property      the property the query in the collection,
 *                                default: _id
 * @return {Function}             the middleware
 */
function exist(pName, collection, type, property) {
  type = typeof type === 'undefined' ? 'params' : type;
  property = typeof property === 'undefined' ? '_id' : property;
  return function (req, res, next) {
    if (['params', 'body', 'query'].indexOf(type) === -1 ) {
      var err = new Error('Unknown request type: ' + type);
      log.error(err);
      return res.status(500).send(err.message);
    }
    var query = {};
    query[property] = req[type][pName];
    collection.findOne(query).exec(function (err, item) {
      if (err) {
        log.error(err);
        return res.status(500).send(err.message);
      }
      ...
      if (!item) {
        return res.status(404).send('item ' + req[type][pName] + ' not found in ' + collection.collectionName);
      }

      req[req[type][pName]] = item;
      next();
    });
  };
}
